...
def rewrite_lookup_key(model, lookup_key):
    localized_fieldnames = translator.get_options_for_model(
        model).localized_fieldnames

    lang = get_language()

    if lang != mt_settings.DEFAULT_LANGUAGE:
        if localized_fieldnames is not None:
            ...
            # "name_fr__startswith"
            if pieces[0] in localized_fieldnames:
...
def get_fields_to_translatable_models(model):
    ...
    for field_name in model._meta.get_all_field_names():
        ...
        if direct and isinstance(field_object, RelatedField):
            if translator.get_options_for_model(
                    field_object.related.parent_model).localized_fieldnames \
                    is not None:
